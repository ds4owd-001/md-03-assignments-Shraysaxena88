...
```{r}
sanitation_india_notsafelymanaged <- sanitation |> 
...
```{r}
sanitation_india_notsafelymanaged |> 
...
```{r}
ggplot(data = sanitation_india_notsafelymanaged, 
...
1.  Look at the plot that you created. What do you notice about the order of the bars / order of the legend?
    ...
    The bars/legend is always ordered from basic sanitation services to limited to no sanitation facilities to unimproved sanitation facilities as the best- to worst-case scenario.

2.  What would you want to change?

    I would change the order to: basic then limited then unimproved and finally no sanitation facilities going from best- to worst-case.

3.  Why did we remove "safely managed sanitation services" from the data set in Task 3?

    The reason to remove safely managed sanitation services from the data set is not clear to me, since you would want to track the country's progress year-on-year as total sanitation. Plotting along with the safely managed sanitation option would make year-on-year comparison easy as it enables us to keep the 100% mark on the y-axis constant for each year.

    In the case without safely managed sanitation, the labels demonstrate an increase in basic sanitation and reduction in limited and unimproved services for each national, rural and urban residence, but it doesn't show the true change in reaching the goal of safely managed sanitation for all. Nonetheless, the chart does provide a powerful image of progress.
...
2.  What do you observe when you look at the code and plot?
    ...
    The histogram plot shows data from the year 2020 for all countries. The scaling is off on the y-axis (because of scales = "free_y"), making it hard to compare between sanitation types. The code asks for no legend to be shown (legend.position ="none").

    The plot reveals that in the year 2020 (10 years away from the target data), we are still far from the goal of of 100% safely managed sanitation for all and an open defecation free society. A good amount of data (50-100 counts) shows 0% San_OD, 0% San_lim, 0% san_unim, and 100% san_bas, which makes me think that countries are making fast progress towards basic sanitation services but a slow progress in converting basic sanitation to safely managed sanitation (\<10 counts at 100%).
